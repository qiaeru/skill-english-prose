...
- The 1-10 scoring grid. A model grading its own rewrite hands out eights and nines whatever the text, and an eight on every axis cleared the old 35 threshold, so the grid never fired; the count of checks still failing was already the gate. In its place, a final read of four yes-or-no questions on the whole text that no line-by-line check can ask: does it sound like a person, would the author recognize it, does every sentence pass read aloud, is anything left to cut. The delivered text carries the count and the note of what changed, with no score; the README and its quick test follow.
